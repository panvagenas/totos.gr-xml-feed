...
class totos extends framework {
	...
	public function do_your_woo_stuff() {
		...
		$this->©success->forceDBLog( 'product', array(),
			'<strong>Totos XML generation started at ' . date( 'd M, Y H:i:s' ) . '</strong>' );

		$prodInXml = $this->processProducts();

		$this->©success->forceDBLog( 'product', array(),
			'<strong>Totos XML generation finished at ' . date( 'd M, Y H:i:s' ) . '</strong><br>Time taken: ' . round( microtime( true ) - $sTime,
